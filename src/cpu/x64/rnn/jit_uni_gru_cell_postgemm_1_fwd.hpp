            ...
            L(vector_loop_start_label);
            {
                for (size_t loop_ur_idx = 0; loop_ur_idx < loop_ur;
                    ...
                    deq_w(src_data_t, G0, tmp1_vmm, tmp2_vmm,
                            0 * rnn_.dhc + loop_ur_idx * vlen_qscale, mask, true);
                    deq_w(src_data_t, G1, tmp1_vmm, tmp2_vmm,
                            1 * rnn_.dhc + loop_ur_idx * vlen_qscale, mask, true);

                    // apply bias
                    uni_vaddps(G0, G0, B_addr(0, loop_ur_idx));
                    uni_vaddps(G1, G1, B_addr(1, loop_ur_idx));
                }
